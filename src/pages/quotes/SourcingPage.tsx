...
const style = {
  chip: { padding: '5px 12px', borderRadius: 20, border: '1px solid #E2E8F0', background: 'var(--bg-card)', fontSize: 'var(--text-11)', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 500, whiteSpace: 'nowrap' as const, fontFamily: 'inherit' },
  supplierChip: { padding: '6px 14px', borderRadius: 8, background: 'var(--bg-primary)', border: '1px solid #E2E8F0', fontSize: 'var(--text-13)', color: 'var(--text-secondary)', fontWeight: 600 },
};
...
const s: Record<string, React.CSSProperties> = {
  ...
  searchBox: { display: 'flex', alignItems: 'center', background: 'var(--bg-card)', border: '1.5px solid #E2E8F0', borderRadius: 14, padding: '6px 6px 6px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: 14 },
  searchIcon: { fontSize: 'var(--text-15)', marginRight: 4, flexShrink: 0 },
  searchInput: { flex: 1, border: 'none', background: 'none', fontSize: 'var(--text-15)', color: 'var(--text-primary)', padding: '8px 0', fontWeight: 500, minWidth: 0 },
  searchBtn: { background: 'var(--accent-blue)', color: 'var(--bg-card)', border: 'none', borderRadius: 10, padding: '10px 22px', fontSize: 'var(--text-13)', fontWeight: 700, cursor: 'pointer', flexShrink: 0 },
  suggestions: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  error: { background: '#FEF2F2', color: '#DC2626', padding: '12px 16px', borderRadius: 10, marginBottom: 20, fontSize: 'var(--text-13)', fontWeight: 600 },
  loadingState: { display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0', gap: 16 },
  loadingDots: { display: 'flex', gap: 8 },
  dot1: { width: 10, height: 10, borderRadius: 5, background: 'var(--accent-blue)', animation: 'pulse 1s ease infinite' },
  dot2: { width: 10, height: 10, borderRadius: 5, background: 'var(--accent-blue)', opacity: 0.6 },
  dot3: { width: 10, height: 10, borderRadius: 5, background: 'var(--accent-blue)', opacity: 0.3 },
  loadingText: { fontSize: 'var(--text-15)', color: 'var(--text-secondary)', fontWeight: 500 },
  ...
  aiIcon: { fontSize: 18 },
  analysisTitle: { fontSize: 'var(--text-13)', fontWeight: 800, color: '#1E40AF' },
  analysisSummary: { fontSize: 'var(--text-13)', color: '#334155', lineHeight: 1.6, marginBottom: 12 },
  topPick: { fontSize: 'var(--text-13)', color: 'var(--text-primary)', fontWeight: 600, background: 'rgba(255,255,255,0.7)', borderRadius: 8, padding: '8px 12px', marginBottom: 12 },
  topPickLabel: { color: 'var(--accent-blue)' },
  tips: { display: 'flex', flexDirection: 'column', gap: 6 },
  tip: { display: 'flex', gap: 8, fontSize: 'var(--text-13)', color: 'var(--text-secondary)' },
  tipIcon: { flexShrink: 0 },
  resultsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 14 },
  resultCard: { background: 'var(--bg-card)', borderRadius: 14, padding: '18px 18px', border: '1px solid #F1F5F9', boxShadow: 'var(--shadow-card)', display: 'flex', flexDirection: 'column', gap: 8 },
  resultHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 },
  sourceTag: { fontSize: 'var(--text-11)', fontWeight: 700, color: 'var(--accent-blue)', background: '#EFF6FF', padding: '3px 8px', borderRadius: 6 },
  price: { fontSize: 'var(--text-15)', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.5px', whiteSpace: 'nowrap' },
  resultTitle: { fontSize: 'var(--text-13)', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.4 },
  resultSnippet: { fontSize: 'var(--text-13)', color: 'var(--text-secondary)', lineHeight: 1.5, flex: 1 },
  delivery: { fontSize: 'var(--text-11)', color: '#059669', fontWeight: 600 },
  resultFooter: { display: 'flex', gap: 8, marginTop: 4 },
  visitBtn: { flex: 1, textAlign: 'center', padding: '8px', background: 'var(--bg-primary)', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 'var(--text-13)', color: 'var(--text-secondary)', fontWeight: 600, textDecoration: 'none', display: 'block' },
  saveBtn: { padding: '8px 14px', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, fontSize: 'var(--text-13)', color: 'var(--accent-blue)', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' },
  savedBtn: { background: '#ECFDF5', border: '1px solid #A7F3D0', color: '#059669' },
  savedSection: { background: 'var(--bg-card)', borderRadius: 16, padding: '20px 22px', border: '1px solid #F1F5F9', boxShadow: 'var(--shadow-card)' },
  savedTitle: { fontSize: 'var(--text-15)', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 16, letterSpacing: '-0.3px' },
  savedList: { display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 },
  savedItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'var(--bg-primary)', borderRadius: 10 },
  savedInfo: { flex: 1, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', minWidth: 0 },
  savedSource: { fontSize: 'var(--text-11)', fontWeight: 700, color: 'var(--accent-blue)', background: '#EFF6FF', padding: '2px 6px', borderRadius: 4, flexShrink: 0 },
  savedName: { fontSize: 'var(--text-13)', fontWeight: 600, color: 'var(--text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  savedPrice: { fontSize: 'var(--text-13)', fontWeight: 800, color: 'var(--text-primary)', flexShrink: 0 },
  removeBtn: { width: 26, height: 26, border: 'none', background: '#FEE2E2', color: 'var(--accent-red)', borderRadius: 6, fontSize: 'var(--text-11)', cursor: 'pointer', flexShrink: 0 },
  addToQuoteBtn: { width: '100%', padding: '11px', background: 'var(--accent-blue)', color: 'var(--bg-card)', border: 'none', borderRadius: 10, fontSize: 'var(--text-13)', fontWeight: 700, cursor: 'pointer' },
  empty: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '60px 0', color: 'var(--text-tertiary)' },
  emptyState: { display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '60px 24px' },
  emptyTitle: { fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 10, letterSpacing: '-0.5px' },
